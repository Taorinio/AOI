...
def pixelart(path, width, height):
    ...
    for y in range(0, image.height, image.height // height):
        for x in range(0, image.width, image.width // width):
            ...
            prearray.append(color)
        array.append(tuple(prearray))
        prearray.clear()
    return tuple(array)
...
clamp = lambda num, minNum, maxNum:  min(max(num, minNum), maxNum)
...
def matrixgrad(array, size = (10, 10)):
    matrix = []
    for i, j in zip(gradient(array[0], size[0]), gradient(array[1], size[0])):
        matrix.append(gradient([i, j], size[1]))
